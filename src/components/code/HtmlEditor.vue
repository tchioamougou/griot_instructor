<template>
    ...
    >
        ...
        <div class="flex items-center space-x-2">
            ...
            <div class="flex bg-gray-800 rounded overflow-hidden ml-2">
                ...
                :class="(!isPreview)?' bg-primary':''"
                @click="codeDer">Code</button>
                <button class="px-3 py-1 text-gray-300 hover:bg-gray-700 transition" @click="preview" :class="(isPreview)?' bg-primary':''">Aperçu</button>
            </div>
            <button
 ...
 <!-- Barre de progression sous le header -->
    <div v-if="showProgress" class="w-full bg-gray-200 h-2 rounded-b">
        <div
            class="bg-blue-600 h-2 rounded-b transition-all duration-200"
            :style="{ width: progress + '%' }"
        ></div>
    </div>
    <div
        class="w-full md:h-96 bg-gray-800 shadow-xl overflow-hidden mb-6 flex flex-col md:flex-row"
        v-if="isPreview"
        :class="isFullscreen ? 'flex-1 h-full' : ''"
    >
        ...
        <div class="w-full p-4 flex-1">
            <iframe id="html-output" ref="iframeRef" title="Prévisualisation HTML"
                class="w-full h-64 md:h-[500px] bg-white rounded-md border border-gray-700"
                :class="isFullscreen ? 'h-full min-h-[400px]' : ''"
                sandbox="allow-scripts allow-forms allow-popups allow-modals allow-same-origin"></iframe>
        </div>
    </div>
    <div
        class="w-full bg-gray-800 overflow-hidden mb-6 flex flex-col md:flex-row"
        v-else
        :class="isFullscreen ? 'flex-1 h-full' : ''"
    >
        ...
        <div class="w-full p-4 border-b md:border-b-0 md:border-r border-gray-700 flex-1">
            <textarea id="code-input"
                ...
                :class="isFullscreen ? 'h-full min-h-[400px]' : ''"
                v-model="code" spellCheck="false" placeholder="Écrivez votre code HTML ici..."></textarea>
        </div>
    </div>
</template>
