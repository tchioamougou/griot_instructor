    ...
    >
      ...
      <div class="relative max-h-[195px]">
        ...
        <span
          ...
          >+{{result.revenueGrowthPercent}}%</span
        >
      </div>
      <p class="mx-auto mt-1.5 w-full max-w-[380px] text-center text-sm text-gray-500 sm:text-base" >
        {{ $t('monthlySales.message', { amount: result.monthRevenue }) }}
      </p>
    </div>
...
import { getInstructorStatRevenue } from "@/services/griot_service";
...
const menuItems = [
  { label: t('view_more'), onClick: () => console.log('View More clicked') },
 // { label: 'Delete', onClick: () => console.log('Delete clicked') },
]
// Example usage in a setup function or method
const fetchRevenue = async (instructorId: string) => {
  try {
    const response = await getInstructorStatRevenue(instructorId);
...
const series = computed(() => [result.value.todayRevenue])
...
fetchRevenue(props.instructor.id);
</script>
