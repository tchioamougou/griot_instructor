<template>
    <template v-for="(tb, i) in tabs" :key="i">
        <span>{{ tb.value }}</span>
        <div class="flex flex-col md:flex-row gap-4 mb-0">
            ...
            <div class="md:w-[20%] w-full">
                <ul class="flex md:flex-col flex-row justify-between md:space-y-2 space-x-2  md:space-x-0 list-none">
                    <li v-for="(tab, index) in tb.items" :key="index">
                        <a class="text-md font-sans font-medium flex items-center gap-2 leading-normal cursor-pointer px-4 py-2 rounded-lg text-center w-full flex-wrap"
                            @click="selectTab(index + 1)" :class="{
