...
const uriEnrollments = import.meta.env.VITE_APP_DOMAIN_GRIOT_DATA_BASE + 'enrollments'
const uriReview = import.meta.env.VITE_APP_DOMAIN_GRIOT_DATA_BASE + 'reviews'
const uriPractice = import.meta.env.VITE_APP_DOMAIN_GRIOT_DATA_BASE + 'practices'
const uriQuestion = import.meta.env.VITE_APP_DOMAIN_GRIOT_DATA_BASE + 'questions'
const uriPracticePlay = import.meta.env.VITE_APP_DOMAIN_GRIOT_DATA_BASE + 'practicePlays'
const uriInstructors = import.meta.env.VITE_APP_DOMAIN_GRIOT_DATA_BASE + 'instructors'
const uriStudents = import.meta.env.VITE_APP_DOMAIN_GRIOT_DATA_BASE + 'students'
const uriChats = import.meta.env.VITE_APP_DOMAIN_GRIOT_DATA_BASE + 'chats'
const uriMessages = import.meta.env.VITE_APP_DOMAIN_GRIOT_DATA_BASE + 'messages'
...
const uriCourseQuestion = import.meta.env.VITE_APP_DOMAIN_GRIOT_DATA_BASE + 'course/questions'
...
export const getInstructorsTop = (category: string, subCategory: string, topic: string) => {
  return fetch(
    `${uriUser}/instructors?category=${category}&subCategory=${subCategory}&topic=${topic}`,
  )
}
export const getUserByRoles = (
  roleId: string,
  word: string,
  pageNumber: number,
  pageSize: number,
  ...
  return fetch(
    `${uriUser}/roles?roleId=${roleId}&word=${word}&pageNumber=${pageNumber}&pageSize=${pageSize}`,
  )
}
export const getUserMonthStat = (id: string, pageNumber: number, pageSize: number) => {
  return fetch(`${uriUser}/${id}/stat?pageNumber=${pageNumber}&pageSize=${pageSize}`)
}
...
export const updateWishList = (id: string, courseId: string, action: string) => {
  return fetch(`${uriUser}/${id}/wishList?courseId=${courseId}&action=${action}`, {
    method: 'PUT',
    headers: {
      Accept: 'application/json',
      'Content-Type': 'application/json',
...
export const getCourseByIdPreview = (courseId: string, section: any) => {
  const currency =
    storeUser.user && storeUser.user.currencyCode ? storeUser.user.currencyCode : 'eur'
...
export const searchAutoCompleted = (word: string) => {
  return fetch(`${uriCourse}/search_autocompleted?word=${word}`)
}
...
export const getCoursesProgressionById = (
  id: string,
...
export const updateFavorites = (id: string, courseId: string, isFavorite: boolean) => {
  return fetch(`${uriUser}/${id}/courses/${courseId}/favorites?isFavorite=${isFavorite}`, {
    method: 'PUT',
    headers: {
      Accept: 'application/json',
      'Content-Type': 'application/json',
...
export const updateArchived = (id: string, courseId: string, isArchived: boolean) => {
  return fetch(`${uriUser}/${id}/courses/${courseId}/archived?isArchived=${isArchived}`, {
    method: 'PUT',
    headers: {
      Accept: 'application/json',
      'Content-Type': 'application/json',
...
export const getMessages = (chatId: string, pageNumber: number, pageSize: number) => {
  return fetch(`${uriChats}/${chatId}/messages?pageNumber=${pageNumber}&pageSize=${pageSize}`, {
    method: 'GET',
  })
}
...
export const calculateWebCartPrice = async (cart: any) => {
  const response = await fetch(
    `${cartPriceUri}/charge/price?webcart=${cart.webcart}&user=${cart.user}`,
    {
      headers: {
        'Content-Type': 'application/json',
      },
...
/// this function is used to get cours questions

export const getCoursesQuestions = async (questionId: string) => {
  const response = await fetch(`${uriCourseQuestion}/${questionId}/details`, {
    headers: {
      Accept: 'application/json',
      'Content-Type': 'application/json',
    },
...
export const getCoursesQuestionsFilter = async (courseIds: any) => {
  const response = await fetch(`${uriCourseQuestion}/details`, {
    headers: {
      Accept: 'application/json',
      'Content-Type': 'application/json',
    },
...
export const questionCourseAnswerPost = async (request: any) => {
